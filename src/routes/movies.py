...
def get_movie(response: Response, movie_id: int) -> dict:
...
def post_movie(response: Response, data: dict = Body(...)) -> dict:
    try:
        add_movie(data)
...
def update_movie(movie_id: int, response: Response, data: dict = Body(...)) -> dict:
    try:
        res = update_movie_by_id(movie_id, data)
        if res:
...
def delete_movie(movie_id: int, response: Response) -> dict:
    try:
        res = delete_movie_by_id(movie_id)
        if res:
